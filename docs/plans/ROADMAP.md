...
`last_reviewed: 2026-07-28 (extraction defect ADR-038; correlated draws ADR-039)` · **NOTHING IS BLOCKED (ADR-025). The work order is `docs/HARDWARE_SESSION.md`; every measurement it produces has somewhere to land, and the build chain is `tools/build_all.py`.**
...
| **5 · S4 + S6** | ✅ **DONE** | S4 Jan 15 2026 (31 pp) + S6 snapshot acquired 2026-07-25; A2/A3/A4/A5/A6 resolved; **55** rules cited in `docs/citations.json`. **Re-verified byte-identical at source 2026-07-27**, and an extraction defect that had hidden §5.5–§5.13 was found and fixed 2026-07-28 (**ADR-038**) — the nine recovered rules are in `PHASE7_CONSTRAINTS.md` §6c | — |
